...
@Component({
  ...
  styles: [`
    ...
    .sv-status.on .ic{background:rgba(16,185,129,.14);color: var(--wx-success);}
    .sv-status.off .ic{background:rgba(148,163,184,.16);color: var(--wx-text-muted);}
    .sv-status b{display:block;font-size:14px;font-weight: 600;}
    .sv-status small{display:block;font-size:12px;color: var(--wx-text-muted);margin-top:2px;}
  ...
  <div class="panel-content">
    <div class="section-title"><i class="ph ph-device-mobile"></i> Pago de servicios y recargas</div>
    ...
    <div class="sv-status" [ngClass]="enabled ? 'on' : 'off'">
      <span class="ic"><i class="ph" [ngClass]="enabled ? 'ph-fill ph-check-circle' : 'ph-plug'"></i></span>
      <div>
    ...
    <ng-container *ngIf="!enabled">
      ...
      <div class="btn-row" style="margin-top:1.2rem;">
        <button class="btn-primary" type="button" (click)="conectar()" [disabled]="busy || !key || !nip">
          <i class="ph ph-link-simple"></i> {{ busy ? 'Validando...' : 'Conectar' }}
        </button>
      </div>
    </ng-container>

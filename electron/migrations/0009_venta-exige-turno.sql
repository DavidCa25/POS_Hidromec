/* 0009_venta-exige-turno.sql
 * ---------------------------------------------------------------------------
 * Sin turno abierto en la caja no se registra ninguna venta.
 *
 * QUE PASABA
 * ----------
 * `sp_register_sale` solo exigia turno para el EFECTIVO, y por un motivo
 * tecnico: el turno hacia falta para colgar el movimiento de caja. Una venta
 * con tarjeta, transferencia, terminal o a credito no genera ese movimiento,
 * asi que entraba sin turno y quedaba fuera del corte del dia.
 *
 * El turno no es un detalle del efectivo: es a quien pertenece la venta.
 *
 * QUE CAMBIA
 * ----------
 * La comprobacion se hace para CUALQUIER forma de pago, antes de abrir la
 * transaccion -no hay nada que deshacer- y sigue siendo por caja:
 * `cash_closures` con `register_id` y `closed_at IS NULL`.
 *
 * La guarda del efectivo se conserva tal cual: sigue necesitando el id del
 * turno concreto para el movimiento de caja.
 *
 * CONSECUENCIA A TENER EN CUENTA
 * ------------------------------
 * Una instalacion que hoy venda con tarjeta sin abrir turno dejara de poder
 * hacerlo. Es el contrato pedido, pero es un cambio de comportamiento real y
 * no un arreglo invisible.
 * ---------------------------------------------------------------------------
 */
